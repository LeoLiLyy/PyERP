...
# change this to true if you need to enable debug mode
debug = True
...
app = Flask(__name__)
# generating secret key for creating sessions
...
def check_in_item():
    if request.method == 'POST':
        ...
        new_item = Inventory(ItemID=id, I_Name=name, I_Disc=disc, InDate=date, OutDate=od, Warehouse_Num=warehouse_num,
                             Quantity=quantity)
...
def inventory():
    ...
    if item_id:
        if action == 'i_edit':
            ...
            return redirect("/inventory")
    return render_template(escape('html/inventory.html'), items=escape(items_in), o_items=item_out, total_items=total_items_in,
                           available_space=free_space)
